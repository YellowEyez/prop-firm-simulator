...
def main():
    ...
    with col_title:
        st.title("Project StarBase")
        st.markdown("*Auditable TradingView ingestion today; full prop-firm household lifecycle engine under cumulative development.*")
    
    st.divider()

    # StarBase mode selector. v2 audit mode is isolated from the legacy lifecycle engine.
    with st.sidebar:
        st.header("🚀 StarBase Mode")
        starbase_mode = st.radio(
            "Choose workspace",
            ["TradingView Import + Audit (v2)", "Legacy Simulator (reference only)"],
            index=0,
            help="The v2 audit path is the trusted source-ingestion gate. The legacy lifecycle simulator is preserved for reference until v4+ rewrites its rule/state engine."
        )
