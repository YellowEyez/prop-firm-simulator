...
def render_historical_runner_page() -> None:
    ...
    if not strict_for_capacity.empty:
        ...
        with st.expander("Fleet capacity preview (bridge to v5)", expanded=True):
            ...
            st.caption("v4B still trades one account. These slot counts show the fleet size v5 would need to mechanically consume the corresponding share of this exact strategy's historical signal stream.")
